...
async function getStripeCredentials(): Promise<{ secretKey: string; webhookSecret?: string }> {
  ...
  if (!settings?.secret_key) throw new Error("Stripe is not connected or has no secret key.");
...
export async function getStripeWebhookSecret(): Promise<string> {
  ...
  if (!webhookSecret) throw new Error("Stripe webhook secret is not available.");
